...
export default function InstagramRadarView({ cliente, startDate, endDate }) {
  ...
  const PostCard = ({ post, type }) => {
    ...
    return (
      <div className="bg-zinc-900 rounded-[2rem] border border-zinc-800 overflow-hidden flex flex-col group hover:border-amber-500/30 transition-all shadow-2xl">
        ...
        <div className="p-6 flex-1 flex flex-col gap-4">
          ...
          <div className="mt-auto pt-4 space-y-4">
            {analysis ? (
              <div className="bg-amber-600/10 border border-amber-500/20 rounded-2xl p-4 space-y-3 animate-in fade-in slide-in-from-top-2">
                <div className="flex justify-between items-start">
                  <div>
                    <span className="text-[8px] font-black text-amber-500 uppercase tracking-widest block mb-1">Estratégia Recomendada</span>
                ...
                <div className="pt-2 border-t border-amber-500/10">
                  ...
                  <p className="text-[11px] text-zinc-200 font-bold">{analysis.pergunta_estrategica}</p>
                </div>
              </div>
            ) : (
              <button 
                ...
                className="w-full py-4 bg-zinc-800 hover:bg-amber-600 hover:text-white rounded-2xl text-[10px] font-black uppercase tracking-widest text-zinc-400 transition-all flex items-center justify-center gap-2 border border-zinc-700"
              >
                {isAnalyzing ? (
    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-6">
        <div className="space-y-2">
          <p className="text-amber-500 text-[10px] font-black uppercase tracking-[0.4em]">Content Monitoring</p>
          <h1 className="text-5xl font-black text-white tracking-tighter uppercase flex items-center gap-4">
            Radar Instagram <Instagram className="text-amber-500" size={40} />
          </h1>
          <p className="text-zinc-500 font-medium uppercase text-[10px] tracking-widest flex items-center gap-2">
            <Target size={12} className="text-amber-500/50" /> Identificando oportunidades de escala em tempo real
          </p>
        </div>

        <div className="flex flex-col items-end gap-3">
          <div className="flex items-center gap-3 bg-zinc-900/80 p-2 rounded-3xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.3)] backdrop-blur-2xl">
            <div className="flex items-center gap-3 px-4 py-2.5 bg-zinc-950/40 rounded-2xl border border-white/5 focus-within:border-amber-500/40 focus-within:bg-zinc-950/60 transition-all duration-300">
              <Calendar size={14} className="text-amber-500" />
              <input 
                type="date" 
                value={localStartDate} 
                onChange={e => setLocalStartDate(e.target.value)} 
                className="bg-transparent text-[11px] font-black text-zinc-200 outline-none w-28 uppercase tracking-tighter [color-scheme:dark]" 
              />
            </div>
          ...
          <button 
            onClick={fetchPosts} 
            disabled={loading}
            className="p-3 bg-amber-600/10 text-amber-500 rounded-xl hover:bg-amber-600 hover:text-white transition-all ml-2"
          >
      ...
      {loading ? (
        <div className="py-40 flex flex-col items-center justify-center gap-6">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500">Sincronizando com a conta comercial...</p>
        </div>
      ) : error ? (
